...
final class TestClassInfoTask implements CancellableTask<CompilationController> {
    ...
    private static String ANNOTATION_TEST = "org.junit.Test"; //NOI18N
    private static String ANNOTATION_SPEC = "com.funambol.bfdunit.Spec"; //NOI18N
    ...
    public void run(CompilationController controller) throws Exception {
		...
		if (junit3) {
			...
			typeElement = (TypeElement) annotationMirror.getAnnotationType().asElement();
                        if (
                            typeElement.getQualifiedName().contentEquals(ANNOTATION_TEST) ||
                            typeElement.getQualifiedName().contentEquals(ANNOTATION_SPEC)
                        ) {
			    methodName = mn;
			    break;  
			}
